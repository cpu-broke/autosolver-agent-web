...
const RAW_ROOT_SAMPLE_URL = 'https://raw.githubusercontent.com/cpu-broke/autosolver-agent-web/main/large_seed301.txt';
...
function getSampleUrls() {
  ...
  return [
    ...
    new URL(ROOT_SAMPLE_PATH, window.location.href).href,
    repoName ? `/${repoName}/${SAMPLE_PATH}` : `/${SAMPLE_PATH}`,
    repoName ? `/${repoName}/${ROOT_SAMPLE_PATH}` : `/${ROOT_SAMPLE_PATH}`,
    RAW_SAMPLE_URL,
    RAW_ROOT_SAMPLE_URL,
  ];
}

async function fetchTextWithTimeout(url, timeout = 3500) {
  const sampleController = new AbortController();
  const timer = window.setTimeout(() => sampleController.abort(), timeout);
  try {
    const res = await fetch(url, { cache: 'reload', signal: sampleController.signal });
...
async function loadCompleteSample() {
  ...
  console.warn('示例数据加载失败，已尝试：', triedUrls, lastError);
...
function bindEvents() {
  ...
  loadSampleBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      showToast(`${error.message || '示例加载失败'}，请确认 sample/large_seed301.txt 或 large_seed301.txt 已上传。`, 6200);
    } finally {
