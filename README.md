...
normal          默认普通场景，由其他标记反推得到
```

这些标记用于控制后续策略分支。它们不是完全互斥的标签，`solve()` 会在原始统计结果基础上进一步生成 `search_st`，把强稀缺和路线稀缺合并到搜索用的稀缺判断中。

需要注意的是，`normal` 不是 `_stats()` 直接返回的字段，而是主流程中的默认情况。通常可以理解为：

```text
不是 low
不是 route_scarce
且不触发小规模专用精确分支时，按普通场景处理
```

如果数据同时不是低概率、不是路线覆盖不足，也没有被搜索状态判定为强稀缺，那么后续主要走普通候选池和普通多骑手输出逻辑。
...
`normal` 场景下不会额外启用低概率专用候选，也不会启用路线稀缺专用覆盖搜索。它更依赖单骑手匹配基线、可用时的多排序贪心、普通多骑手备选构造，以及后续固定骨架重分配和局部窗口修复。
...
中小规模普通场景：
  使用 _mcmf_single()
  可使用 _greedy_candidates()
  若 task_count <= 18，则额外使用小规模精确搜索

大规模普通场景：
  使用 _mcmf_single()
  跳过 _greedy_candidates()
  使用 _safe_output_multi() 快速构造多骑手输出
  进入普通后处理修复
```
